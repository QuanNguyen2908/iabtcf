...
const vendorlistJson22: any =
  vendorListJson22 as unknown as VersionOrVendorList;
...
describe("GVL", (): void => {
    ...
    Object.keys(json).forEach((key: string): void => {
      ...
      if (key === "lastUpdated") {
        expect((gvl[key] as Date).getTime(), msg).to.equal(
          new Date(json.lastUpdated).getTime()
        );
  ...
  it("should not create GVL object because with a invalid language", (): void => {
    ...
    assert.throws(
      () => new GVL(vendorlistJson22, { language: "EQ" }),
      GVLError
    );
  });
  ...
  for (const providedLang in langToTranslationsDataProvider) {
    it(
      "should replace the language when changeLanguage() is called with most close to valid language for: " +
        providedLang,
      async (): Promise<void> => {
        GVL.baseUrl = "http://sweetcmp.com";
        ...
        expect(req.url).to.equal(
          GVL.baseUrl +
            GVL.languageFilename.replace(
              "[LANG]",
              `${langToTranslationsDataProvider[providedLang]}`
            )
        );
  ...
  it("should replace the language when changeLanguage() is called with same language with different variant", () => {
    ...
    const firstReq: sinon.SinonFakeXMLHttpRequest =
      XMLHttpTestTools.requests[0];

    expect(firstReq.url).to.equal(
      GVL.baseUrl +
        GVL.languageFilename.replace("[LANG]", language.toLowerCase())
    );
    ...
    const secondReq: sinon.SinonFakeXMLHttpRequest =
      XMLHttpTestTools.requests[1];

    expect(secondReq.url).to.equal(
      GVL.baseUrl +
        GVL.languageFilename.replace("[LANG]", secLang.toLowerCase())
    );
  });
