...
describe("encoder/field->PurposeRestrictionVectorEncoder", (): void => {
  ...
  it("should create restriction groups and all with vendor range if only consecutive vendors from the GVL with narrowed vendors", (): void => {
    const narrowVendors: number[] = randomize([
      173, 1204, 458, 855, 1228, 788, 1189, 885, 136, 251, 178, 1060, 667, 138,
      157, 517, 1020, 1025, 686, 737, 438, 1145, 956, 1126, 1122, 1003, 459,
      948, 92, 58, 40, 830, 767, 649, 57, 953, 37, 377, 539, 1019, 50, 790, 39,
      14, 93, 22, 81, 264, 565, 384, 224, 6, 66, 507, 410, 195, 27, 259, 785,
      793, 148, 23, 780, 733, 354, 797, 394, 783, 561, 598, 956, 46, 907, 647,
      983, 771, 801, 12, 508, 30, 87, 212, 462, 128, 185, 86, 625, 94, 163, 422,
      792, 329, 945, 315, 810, 243, 285, 416, 77, 56, 591, 630, 868, 91, 1026,
      875, 938, 440, 209, 707, 1029, 735, 852, 126, 1075, 434, 584, 110, 796,
      168, 929, 8, 213, 183, 24, 1050, 312, 1, 120, 795, 100, 78, 323, 159, 119,
      262, 870, 731, 328, 845, 758, 536, 967, 580, 755, 657, 98, 295, 1031, 881,
      856, 431, 316, 131, 921, 606, 253, 10, 730, 946, 333, 452, 150, 278, 436,
      991, 252, 294, 62, 747, 528, 63, 544, 424, 871, 67, 804, 254, 97, 109, 95,
      511, 846, 153, 1081, 52, 102, 599, 228, 142, 475, 152, 703, 101, 230, 302,
      898, 72, 34, 982, 521, 129, 468, 812, 373, 31, 509, 241, 883, 602, 69,
      488, 385, 772, 559, 164, 139, 361, 766, 903, 140, 177, 297, 887, 952, 226,
      76, 808, 835, 11, 60, 290, 192, 800, 787, 922, 759, 232, 71, 4, 16, 506,
      350, 371, 84, 80, 111, 276, 261, 68, 82, 161, 45, 115, 531, 246, 134, 937,
      61, 857, 104, 590, 13, 1077, 655, 165, 114, 484, 275, 42, 89, 786, 132,
      44, 345, 577, 382, 21, 819, 423, 28, 242, 1015, 829, 831, 36, 162, 25,
      512, 7, 744, 190, 380, 1009, 774, 284, 282, 281, 32, 70, 154, 210, 301,
    ]);
    ...
    restrictedPurposeIds.forEach((purposeId) => {
      ...
      const isARange: boolean = BooleanEncoder.decode(
        encoded.substr(index, BitLength.anyBoolean)
      );
      expect(isARange, "isARange").to.equal(true);
      index += BitLength.anyBoolean;

      const startVendorId: number = IntEncoder.decode(
        encoded.substr(index, BitLength.vendorId),
        BitLength.vendorId
      );
  ...
  it("should encode and decode and have the same output", (): void => {
    ...
    const decodedPRV: PurposeRestrictionVector =
      PurposeRestrictionVectorEncoder.decode(encoded);
